...
public class TopoMapStage extends PApplet implements FilterListener,
    ...
    public void drawStops() {
        ...
        for (i = 0; i < NumOfStops; ++i) {
            ...
            Stop stop = mStops[i];
            stop.draw();
            if (stop.delay != null && stop.delay[index] >= MinDelay
                    && stop.delay[index] <= MaxDelay) {
    ...
    public void drawClickedStop() {
        ...
        fill(0);
        int delay = 0;
        if (stop.delay != null) delay = stop.delay[mTimer / 120];
        text("Name: " + stop.name, popupLeft + 5, popupTop + 20);
        text("Delay: " + delay, popupLeft + 5, popupTop + 40);
    }
    ...
    public static String Date = "2011-11-26";
    ...
    public void filterChanged() {
        ...
        MaxDelay = Dashboard.getMaxDelay();
        MinDelay = Dashboard.getMinDelay();
        String newDate = Dashboard.getDay();
    ...
    public void loadStopDelays() {
        try {
            String[] lines = loadStrings(Date + File.separator
                    + stopdelayfilename);
            int linelength = lines.length;
            for (int kk = 0; kk < linelength; ++kk) {
                String[] split = lines[kk].split(";");
                // if (stop2int.containsKey(split[0])) {
                ...
                // } else {
                // System.out.println("Cannot find " + split[0]);
                // }
            }
            System.out.println("Stop delays done " + linelength);
        } catch (Exception e) {
            System.out.println(e.toString());
        }
    }
